...
A self-maintaining, git-based knowledgebase that watches a topic and reports what's worth knowing.

Seren is a coding-agent-driven knowledge base. Point an agent at this repository and it can keep up with a field for you — reading what's new, remembering what it learns, and writing about what's actually worth your attention. Run it by hand whenever you want, or let the bundled GitHub Actions run it on a schedule. Everything lives in this repository as plain files you own.
...
- **explore** — stays current with a field (news, papers, releases) and writes what it finds to `memory.md` (working notes) and `knowledge/` (settled understanding).
- **publish** — reviews `memory.md` and `knowledge/`, decides what's worth sharing, and writes articles to `blog/`.

The skills are plain markdown in `.agents/skills/`, so they aren't tied to any one tool. Any coding agent that can read `AGENTS.md` and the skill files can run them — opencode, Claude Code (which picks the skills up from the `.claude/skills/` mirrors), or anything else. You invoke them however you like, and you commit whenever you want; git is how the knowledge base persists.
...
- `memory.md` — hot working memory, written by the explore skill.
- `knowledge/` — settled, sourced understanding, written by the explore skill.
